...
/obj/machinery/gunnery/magazine/proc/gunneryGetAmmo()
	if(ammunition > 0)
		ammunition--
		return 1
	return 0
...
/obj/machinery/gunnery/barrel/proc/gunneryAimOffset(target)
	return target
...
/obj/machinery/gunnery/core
	...
	var/ready = 0
	var/schutsound = 'sound/weapons/gunshot/gunshot3.ogg'
...
/obj/machinery/gunnery/core/proc/gunneryTarget(var/atom/target,var/tx,var/ty,var/tz)
	if(!gunneryReady())
		visible_message("WARNING: Not ready to fire!")
		return 0
	if(tx && ty && tz)
...
/obj/machinery/gunnery/core/proc/gunneryFire(var/atom/target)
	if(!ready || !target)
		return 0
	if(gBarrel)
		target = gBarrel.gunneryAimOffset(target)
	if(gMag)
		if(gMag.gunneryGetAmmo())
